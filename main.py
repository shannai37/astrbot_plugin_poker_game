...
@register("astrbot_plugin_poker_game", "山萘", "德州扑克游戏插件 - 支持多人游戏、积分系统、房间管理", "1.0.0")
...
⏰ 操作超时: {timeout} 秒""".format(
    ...
    async def leave_room(self, event: AstrMessageEvent):
        ...
        try:
            ...
            result = await self.room_manager.leave_room(current_room.room_id, user_id)
            if result:
                yield event.plain_result("✅ 已离开游戏")
            else:
                yield event.plain_result("❌ 离开游戏失败")
    ...
    async def game_call(self, event: AstrMessageEvent):
        ...
        try:
            ...
            if room.game.player_action(user_id, PlayerAction.CALL):
                ...
                # 不再在每次操作后发送手牌，只在游戏开始时发送一次
                # try:
                #     await self._send_private_cards(event, user_id, room.game)
                # except Exception as e:
                #     logger.error(f"发送手牌给玩家 {user_id} 失败: {e}")
                
                # 检查游戏是否结束
                if room.game.is_game_over():
                    await self._handle_game_end(room)
            else:
                yield event.plain_result("❌ 无法跟注，请检查游戏状态")
    ...
    async def game_raise(self, event: AstrMessageEvent, amount: int):
        ...
        try:
            ...
            if room.game.player_action(user_id, PlayerAction.RAISE, amount):
                ...
                yield event.plain_result("❌ 无法加注，请检查游戏状态和筹码")
    ...
    async def game_check(self, event: AstrMessageEvent):
        ...
        try:
            ...
            if room.game.player_action(user_id, PlayerAction.CHECK):
                ...
                yield event.plain_result("❌ 无法过牌，请检查游戏状态")
