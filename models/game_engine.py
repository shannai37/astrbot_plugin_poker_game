...
class TexasHoldemGame:
    ...
    def player_action(self, player_id: str, action: PlayerAction, amount: int = 0) -> bool:
        ...
        Returns:
            bool: 操作是否成功
        """
        logger.info(f"处理玩家操作: {player_id} {action.value} {amount}")
        logger.info(f"当前玩家: {self.current_player_id}, 游戏阶段: {self.game_phase.value}")
        
        # 如果游戏已结束，拒绝所有操作
        if self.game_phase == GamePhase.GAME_OVER:
            logger.warning(f"游戏已结束，拒绝操作: {player_id} {action.value}")
            return False
        
        # 移除自动修复调用，避免干扰正常的玩家操作流程
        # self._auto_fix_turn_state()  # 注释掉，防止状态冲突
        ...
        if not self._is_valid_action(player_id, action, amount):
            logger.warning(f"无效操作: {player_id} {action.value} {amount}")
            # 添加详细的状态信息
            if player_id in self.players:
                player = self.players[player_id]
                logger.warning(f"玩家 {player_id} 状态: chips={player.chips}, status={player.status.value}, in_hand={player.is_in_hand()}, can_act={player.can_act()}")
            return False
        ...
        logger.info(f"操作执行完成: {player_id} {action.value}")
        
        # 检查游戏是否已经结束（例如弃牌导致只剩一个玩家）
        if self.game_phase == GamePhase.GAME_OVER:
            logger.info("游戏已结束，停止后续处理")
            return True
        
        # 检查下注轮次是否结束
        round_complete = self._is_betting_round_complete()
        logger.info(f"下注轮次是否完成: {round_complete}")
        
        if round_complete:
            logger.info("进入下一游戏阶段")
            self._advance_to_next_phase()
        else:
            # 轮到下一个玩家
            logger.info("轮转到下一个玩家")
